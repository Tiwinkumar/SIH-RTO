...
$result = mysqli_query($mysqli, "SELECT * FROM ilaw WHERE id='$id'");
...
//echo $status;
  // $sql = 'SELECT cno,accidentdate,psta,numberofvehiclesinvolved,status FROM basic  WHERE psta="$psta"';
   ...
   $sql= "SELECT psta,cno,accidentdate,numberofvehiclesinvolved,status FROM lbasic WHERE la='$id 'and status='Approved From Policestation'";
      ...
      if(mysqli_num_rows($result) > 0) {
         ...
         while($row = mysqli_fetch_array($result)){
            ...
            echo "<td><a href='rto.php?crimenumber=" . $row['cno'] . "'>RTO Details</a></td>";
             echo "<td><a href='final.php?crimenumber=" . $row['cno'] . "'>Samman</a></td>";
