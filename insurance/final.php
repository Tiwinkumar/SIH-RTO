...
$comment=$_POST['comment'];
  ...
  /*  if(empty($firstname) || empty($lastname)) {            
         ...
         */
        //updating the table
        //$result = mysqli_query($mysqli, "UPDATE ic SET psta='$policestationname' , dis='$district' , zone='$zone' , rto='$rto' , ipc='$ipcsection', natureofaccident='$natureofaccident' , placeofaccident='$placeofaccident' , accidentdate='$accidentdate' , accidenttime='$accidenttime' , numberofvehiclesinvolved='$numberofvehicles', collisiontype='$collisiontype' , junctiontype='$junctiontype', junctioncontrol='$junctioncontrol' , roadcategory='$roadcategory' , roadnarrowing='$roadnarrowing' , lightconditions='$lightcondition' , climate='climate', status='$status', la='$la', approval='$approval', comment='$comment' WHERE cno='$crimenumber'");
        
         $sql = "INSERT INTO ic (psta,dis,zone,rto,ipc,natureofaccident,placeofaccident,accidentdate,accidenttime,numberofvehiclesinvolved,collisiontype,junctiontype,junctioncontrol,roadcategory,roadnarrowing,lightconditions,climate,status,la,cno,approval,comment) VALUES ('$policestationname', '$district', '$zone', '$rto', '$ipcsection', '$natureofaccident', '$placeofaccident', '$accidentdate', '$accidenttime', '$numberofvehicles', '$collisiontype', '$junctiontype', '$junctioncontrol', '$roadcategory', '$roadnarrowing', '$lightcondition', '$climate', '$status', '$la', '$crimenumber', '$approval', '$comment')";
...
$result = mysqli_query($mysqli, "SELECT * FROM lbasic WHERE cno='$crimenumber'");
...
<h4><center>(Rule 378 of the Tamil Nadu Motor Vehicles Rules 1989)</center></h4>
    ...
    <div >
     ...
     <input name='la' id='la' value="<?php echo $la;?>" class="form-control" readonly>
      </input>
    </div>
    <div >
     <label><i class="fa fa-user" aria-hidden="true"></i>Insurance Company Status</label>
     ...
     <div >
     <label><i class="fa fa-user" aria-hidden="true"></i>Comment</label>
